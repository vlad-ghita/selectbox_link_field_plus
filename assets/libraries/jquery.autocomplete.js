...
(function ($) {
	...
	$.Autocompleter = function (input, options) {
		...
		function lastWord (value) {
			...
			var cursorAt = $(input).selection().start;
			if (cursorAt == value.length) {words = trimWords(value)} else {words = trimWords(value.replace(value.substring(cursorAt), ""));}
	...
	$.Autocompleter.defaults = {inputClass: "sblp_autocomplete_input", resultsClass: "sblp_autocomplete_results", loadingClass: "sblp_autocomplete_loading", minChars: 1, delay: 400, matchCase: false, matchSubset: true, matchContains: false, cacheLength: 10, max: 100, mustMatch: false, extraParams: {}, selectFirst: true, formatItem: function (row) {return row[0];}, formatMatch: null, autoFill: false, width: 0, multiple: false, multipleSeparator: ", ", highlight: function (value, term) {return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term.replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/gi, "\\$1") + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<strong>$1</strong>");}, scroll: true, scrollHeight: 180};
	...
	$.Autocompleter.Select = function (options, input, select, config) {
		var CLASSES = {ACTIVE: "sblp_autocomplete_over"};
		...
		function movePosition (step) {
			active += step;
			if (active < 0) {active = listItems.size() - 1;} else if (active >= listItems.size()) {active = 0;}
		}
		...
		function fillList () {
			...
			for (var i = 0; i < max; i++) {
				...
				var li = $("<li/>").html(options.highlight(formatted, term)).addClass(i % 2 == 0 ? "sblp_autocomplete_even" : "sblp_autocomplete_odd").appendTo(list)[0];
				$.data(li, "sblp_autocomplete_data", data[i]);
			}
		...
		return{display: function (d, q) {
			...
			term = q;
			fillList();
		}, next:        function () {moveSelect(1);}, prev: function () {moveSelect(-1);}, pageUp: function () {if (active != 0 && active - 8 < 0) {moveSelect(-active);} else {moveSelect(-8);}}, pageDown: function () {if (active != listItems.size() - 1 && active + 8 > listItems.size()) {moveSelect(listItems.size() - 1 - active);} else {moveSelect(8);}}, hide: function () {
			element && element.hide();
			listItems && listItems.removeClass(CLASSES.ACTIVE);
			active = -1;
		}, visible:     function () {return element && element.is(":visible");}, current: function () {return this.visible() && (listItems.filter("." + CLASSES.ACTIVE)[0] || options.selectFirst && listItems[0]);}, show: function () {
			...
			return selected && selected.length && $.data(selected[0], "sblp_autocomplete_data");
