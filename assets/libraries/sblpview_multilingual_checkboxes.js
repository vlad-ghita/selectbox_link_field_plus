(function ($, undefined) {
	...
	sblp.SBLPView_Multilingual_Checkboxes = sblp.SBLPView.extend({
		...
		update: function () {
			...
			view.$view.find("select.target option:selected").each(function () {
				view.$view.find("div.sblp-multilingual_checkboxes input[value=" + $(this).val() + "]").attr("checked", "checked");
			});

			view.$view.find("div.sblp-multilingual_checkboxes input").change(function (e) {
